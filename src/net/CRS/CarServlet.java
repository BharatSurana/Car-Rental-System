...
public class CarServlet extends HttpServlet {
	...
	public void process(HttpServletRequest req,HttpServletResponse res) 
		...
		if(mod.equals("MTN")){
			...
			else if(act.equals("ViewTransSum")){
				session.setAttribute("content_page","transactionSummary.jsp");
				res.sendRedirect("template.jsp");
			}
			else if(act.equals("rDel")){ // rDel
				int rid = Integer.parseInt(req.getParameter("rid"));
				String sql = "DELETE FROM repair where id="+rid;
				DataBaseConnect.updateQuery(sql);
			...
			//	dImage = "drivers/"+dImage;
				...
				System.out.println (sql);
				int update = DataBaseConnect.updateQuery(sql);
				if(update ==1){
			...
			if(act.equals("View")){ // 
				...
				String sql = "INSERT INTO bookings (uid, u_name, v_name, d_name, sdate, edate, nod, km, no_of_seats, status, b_date) "+
					" VALUES ("+uid+",'"+uname+"','"+vname+"','"+uname+"','"+sdate+"','"+edate+"',"+NoOfDays+",'"+KiloMeters+"',"+NoOfSeats+",'Pending',NOW())";
			//	System.out.println (sql);
				int update = DataBaseConnect.updateQuery(sql);
				if(update ==1){
				...
				System.out.println (sql);
				int up=DataBaseConnect.updateQuery(sql);
				if(up ==1){
				...
				//System.out.println (sql);
				int i = DataBaseConnect.updateQuery(sql);			 	
				if(i==1){
